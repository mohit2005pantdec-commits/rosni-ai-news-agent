...
const SOURCE_NAME = 'ABP';
const LIST_URL = 'https://www.abplive.com/agriculture';

async function fetchAbp() {
  ...
  try {
    ...
    const links = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('a'))
        .map(a => a.href)
        .filter(href => href.includes('/agriculture/') && href.length > 40 && !href.includes('/page-') && !href.includes('/page/'))
